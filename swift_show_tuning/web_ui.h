...
<style>
  :root {
    --bg: #07090e; --panel: rgba(16,22,34,.92); --inset: rgba(8,12,20,.75); --line: rgba(255,255,255,.08);
    --cyan: #00f0ff; --amber: #ffaa00; --red: #ff2247; --red-glow: rgba(255,34,71,.5); --green: #00ff88;
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
  ...
  .card { background: var(--panel); border: 1px solid var(--line); border-radius: 20px; box-shadow: 0 12px 32px rgba(0,0,0,.5); }
  ...
  @keyframes pulseAmber { 0%, 100% { box-shadow: 0 0 18px rgba(255,170,0,.4); } 50% { box-shadow: 0 0 36px rgba(255,170,0,.85); } }
  ...
  .pill { display: flex; align-items: center; gap: 6px; font: 800 .6rem var(--disp); letter-spacing: .8px; padding: 6px 11px; border-radius: 20px; background: rgba(0,0,0,.5); border: 1px solid var(--line); color: var(--muted); white-space: nowrap; }
  .pill.up { color: var(--green); border-color: rgba(0,255,136,.4); background: rgba(0,255,136,.1); }
  .pill.down { color: var(--amber); border-color: rgba(255,170,0,.4); }
  ...
  .icon-btn { width: 40px; height: 40px; border-radius: 50%; background: rgba(0,240,255,.12); border: 1px solid rgba(0,240,255,.45); color: var(--cyan); font: 900 1rem var(--disp); }
  .icon-btn:active { background: var(--cyan); color: var(--bg); }
  .demo-bar { display: flex; align-items: center; justify-content: space-between; gap: 10px; padding: 8px 10px 8px 14px; border-radius: 14px; border: 1px dashed rgba(255,170,0,.55); background: rgba(255,170,0,.08); font-size: .8rem; color: #ffd98a; }
  .demo-bar .btn { width: auto; flex-shrink: 0; touch-action: none; }

  /* RPM readout: cut-reason line, big number, slim bar with zones and markers */
  .cluster { padding: 12px 14px 12px; display: flex; flex-direction: column; gap: 10px; }
  .cluster.cutting { border-color: rgba(255,34,71,.6); box-shadow: 0 0 28px rgba(255,34,71,.3); }
  .cut-line { font: 900 .78rem var(--disp); letter-spacing: 1.5px; text-align: center; padding: 8px 10px; border-radius: 12px; border: 1px solid var(--line); background: rgba(0,0,0,.35); color: var(--muted); white-space: nowrap; }
  .cut-line.on { background: rgba(255,34,71,.25); border-color: var(--red); color: #fff; box-shadow: 0 0 16px var(--red-glow); }
  .cut-line.lock { background: rgba(255,170,0,.18); border-color: var(--amber); color: #ffe2a8; }
  .readout { display: flex; justify-content: space-between; align-items: flex-end; gap: 8px; }
  .rpm { font: 700 4.2rem/.85 var(--body); letter-spacing: -1px; font-variant-numeric: tabular-nums; text-shadow: 0 0 14px rgba(0,240,255,.45); }
  .rpm.warn { color: var(--amber); text-shadow: 0 0 18px rgba(255,170,0,.6); }
  .rpm.danger { color: var(--red); text-shadow: 0 0 22px rgba(255,34,71,.8); }
  ...
  .peak { font: 800 .68rem var(--disp); letter-spacing: 1px; color: var(--amber); background: rgba(255,170,0,.12); border: 1px solid rgba(255,170,0,.3); border-radius: 8px; padding: 8px 10px; white-space: nowrap; }
  .pat-mini { font-size: .76rem; font-weight: 700; color: var(--muted); letter-spacing: 1px; }
  .rbar { position: relative; margin-top: 4px; }
  .rtrack { position: relative; height: 12px; border-radius: 6px; overflow: hidden; background: #101622; border: 1px solid rgba(255,255,255,.06); }
  .rtrack i { position: absolute; top: 0; bottom: 0; }
  #zoneWarn { background: rgba(255,170,0,.2); }
  #zoneRed { right: 0; background: rgba(255,34,71,.38); }
  ...
  .seg button { padding: 8px 6px; min-height: 48px; border-radius: 12px; background: rgba(14,20,32,.9); border: 1px solid var(--line); color: var(--muted); display: flex; flex-direction: column; align-items: center; }
  .seg b { font: 800 .66rem var(--disp); letter-spacing: .5px; }
  .seg small { font-size: .72rem; }
  .seg button.active { background: rgba(0,240,255,.14); border-color: var(--cyan); color: var(--cyan); box-shadow: 0 0 14px rgba(0,240,255,.25); }
  .fire-btn { width: 100%; min-height: 104px; padding: 12px 10px 16px; border-radius: 20px; border: 2px solid var(--red); background: linear-gradient(180deg, #2a0b12, #130407); display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 6px; position: relative; overflow: hidden; box-shadow: 0 8px 28px rgba(255,34,71,.3), inset 0 1px 0 rgba(255,255,255,.15); transition: transform .05s, background .1s; -webkit-touch-callout: none; }
  .fire-btn.show { touch-action: none; }
  .fire-btn::before { content: ''; position: absolute; inset: 0; background: repeating-linear-gradient(45deg, rgba(255,255,255,.02) 0 8px, transparent 8px 16px); pointer-events: none; }
  ...
  .fire-cd { position: absolute; left: 0; right: 0; bottom: 0; height: 7px; background: rgba(0,0,0,.45); opacity: 0; }
  ...
  .fire-btn.pressed { background: var(--red); transform: translateY(3px) scale(.985); box-shadow: 0 0 45px rgba(255,34,71,.95); }
  ...
  .fire-btn.holding { background: var(--red); border-color: #fff; box-shadow: 0 0 50px rgba(255,34,71,.95); animation: flash .12s infinite alternate; }
  .fire-btn.fired { background: linear-gradient(180deg, #053315, #02170a); border-color: var(--green); box-shadow: 0 0 35px rgba(0,255,136,.7); }
  ...
  .ls-steps span { font: 800 .58rem var(--disp); letter-spacing: .8px; text-align: center; padding: 6px 0; border-radius: 8px; border: 1px solid var(--line); color: #56627a; background: rgba(0,0,0,.35); }
  .ls-steps span.on { color: var(--c, #fff); border-color: var(--c, #6b7a96); }
  .ls-steps span.on[data-s="2"] { background: var(--red); color: #fff; }
  .warn-line { font-size: .85rem; color: #ffd98a; background: rgba(255,170,0,.1); border: 1px solid rgba(255,170,0,.35); border-radius: 10px; padding: 8px 10px; }
  ...
  .val { font: 700 1.1rem var(--body); padding: 3px 10px; border-radius: 8px; background: rgba(0,0,0,.55); border: 1px solid var(--line); color: var(--amber); font-variant-numeric: tabular-nums; white-space: nowrap; }
  .val.danger { color: var(--red); }
  .sl { display: flex; flex-direction: column; gap: 8px; }
  .sl.off { opacity: .45; }
  .sl-row { display: flex; align-items: center; gap: 8px; }
  .step { width: 44px; height: 44px; flex-shrink: 0; border-radius: 12px; background: rgba(255,255,255,.07); border: 1px solid var(--line); font: 700 1.4rem/1 var(--body); }
  .step:active { background: var(--cyan); color: #000; }
  input[type=range] { -webkit-appearance: none; appearance: none; flex: 1; min-width: 0; height: 10px; border-radius: 6px; border: 1px solid rgba(255,255,255,.06); background: linear-gradient(90deg, rgba(255,34,71,.75) var(--p, 50%), #141b2a var(--p, 50%)); }
  input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 28px; height: 28px; border-radius: 50%; background: linear-gradient(135deg, #ff416c, #ff2247); border: 2px solid #fff; box-shadow: 0 0 14px rgba(255,34,71,.7); }
  ...
  .pat { background: rgba(14,18,28,.85); border: 1px solid var(--line); border-radius: 12px; padding: 10px 8px; display: flex; flex-direction: column; align-items: center; gap: 3px; text-align: center; }
  ...
  .pat.active { background: linear-gradient(180deg, #2b1016, #19090d); border-color: var(--red); box-shadow: 0 0 16px rgba(255,34,71,.35); }
  ...
  .seq i.L { width: 60px; }
  .save-btn { padding: 12px 14px; min-height: 56px; border-radius: 14px; border: 1px solid rgba(255,255,255,.1); background: linear-gradient(180deg, #1c2538, #131926); display: flex; flex-direction: column; align-items: center; gap: 2px; box-shadow: 0 4px 15px rgba(0,0,0,.4); }
  ...
  /* Buttons, forms, firmware update */
  .btn { width: 100%; min-height: 46px; padding: 10px 12px; border-radius: 12px; border: 1px solid rgba(0,240,255,.5); background: rgba(0,240,255,.08); color: var(--cyan); font: 800 .72rem var(--disp); letter-spacing: 1px; }
  .btn:active { background: rgba(0,240,255,.25); }
  .btn:disabled { opacity: .4; }
  .btn.go { background: linear-gradient(135deg, #00ff88, #00aa55); border: none; color: var(--bg); font-weight: 900; box-shadow: 0 0 16px rgba(0,255,136,.35); }
  .btn.warn { border-color: rgba(255,170,0,.45); color: var(--amber); background: rgba(255,170,0,.08); }
  ...
  .ota-st { border-radius: 10px; padding: 9px 11px; font-size: .88rem; line-height: 1.35; background: rgba(0,240,255,.06); border: 1px solid rgba(0,240,255,.25); color: #cfe9f5; display: flex; flex-direction: column; gap: 7px; word-break: break-word; }
  .ota-st.ok { background: rgba(0,255,136,.07); border-color: rgba(0,255,136,.4); color: #b8ffd9; }
  .ota-st.go { background: rgba(0,255,136,.1); border-color: var(--green); color: #fff; }
  .ota-st.warn { background: rgba(255,170,0,.08); border-color: rgba(255,170,0,.45); color: #ffe0a0; }
  .ota-st.err { background: rgba(255,34,71,.08); border-color: rgba(255,34,71,.5); color: #ffc2cd; }
  .bar { display: none; height: 8px; border-radius: 5px; background: rgba(0,0,0,.6); overflow: hidden; }
  ...
  .toast { position: fixed; left: 50%; top: calc(12px + env(safe-area-inset-top)); transform: translate(-50%, -20px); width: max-content; max-width: calc(100% - 24px); text-align: center; background: rgba(10,15,25,.97); border: 1px solid var(--green); color: var(--green); font: 700 .74rem/1.35 var(--disp); letter-spacing: .6px; padding: 11px 18px; border-radius: 22px; box-shadow: 0 10px 30px rgba(0,0,0,.6); opacity: 0; transition: .25s; pointer-events: none; z-index: 3000; }
  .toast.show { opacity: 1; transform: translate(-50%, 0); }
  .toast.warn { border-color: var(--amber); color: var(--amber); }
  .toast.err { border-color: var(--red); color: #ffc2cd; }

  /* Help modal */
  .modal { position: fixed; inset: 0; background: rgba(4,7,12,.9); z-index: 2000; display: flex; justify-content: center; align-items: center; padding: 12px; opacity: 0; pointer-events: none; transition: opacity .2s; }
  .modal.open { opacity: 1; pointer-events: auto; }
  .modal-card { background: #0e1420; border: 1px solid rgba(0,240,255,.35); border-radius: 22px; width: 100%; max-width: 440px; max-height: 90vh; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,.85); }
  ...
  .msec { background: var(--inset); border: 1px solid rgba(255,255,255,.06); border-radius: 14px; padding: 12px 14px; }
...
<div class="wrap">
  ...
  <!-- Top bar: brand, firmware version, connection -->
  ...
  <!-- RPM readout: what is cutting, big number, bar (zones follow the configured redline) -->
  ...
  <!-- Settings (slider blocks are built from SLIDERS in the script) -->
    ...
    <details class="card" id="grpSound" open>
      ...
      <div class="cb">
        <div class="pats" id="pats">
          ...
          <button class="pat" data-p="4" data-seq="Looo"><b>💣 ÁGYÚLÖVÉS</b><small>~1,6 s teljes tiltás, majd visszajön a szikra = nagy dörrenés</small></button>
        </div>
  ...
  <!-- Firmware update: three paths, most convenient first -->
  <details class="card" id="grpFw">
    ...
    <div class="cb">
      ...
      <div class="box">
        ...
        <p class="hint">A telefon letölti a legújabb firmware-t a GitHubról (mobilneten), és feltölti a vezérlőre. Androidon az autó Wi-Fi-je mellett is működik, ha be van kapcsolva a mobilnet; iPhone-on gyakran nem – ott a Wi-Fi-s módot vagy a kézi feltöltést használd.</p>
        <div class="btn-row">
      ...
      <details class="box" id="p2Box">
        ...
        <div class="sub-b">
          <p class="hint">Az ESP egy internetes Wi-Fi-re (otthoni router vagy telefonos hotspot) csatlakozik, és maga tölti le a frissítést. Közben az autó Wi-Fi-je egy pillanatra megszakadhat (csatornaváltás) – az oldal magától újracsatlakozik. Hotspotnál a telefon általában nem maradhat közben az autó Wi-Fi-jén: indítsd el a telepítést, az ESP egyedül befejezi, utána csatlakozz vissza a Swift-PopsAndBangs hálózatra, és nyisd meg újra az oldalt.</p>
          ...
          <div class="row"><div class="lbl"><b>Automatikus ellenőrzés</b><small>Az ESP magától keres új verziót a mentett Wi-Fi-n át</small></div><label class="sw"><input type="checkbox" id="espAuto" data-track aria-label="Automatikus ellenőrzés"><span></span></label></div>
      ...
      <details class="box" id="p3Box">
        ...
        <div class="sub-b">
          <p class="hint">Tartalék megoldás: ha a firmware.bin már a telefonodon van, itt kiválaszthatod és feltöltheted.</p>
  ...
  <details class="card" id="grpSys">
    ...
    <div class="cb">
      ...
      <div class="btn-row">
        <button class="btn sm" onclick="resetPeak()">CSÚCS NULLÁZÁSA</button>
        <button class="btn sm" onclick="openModal()">SÚGÓ</button>
      </div>
  ...
  <div class="modal-card" role="dialog" aria-modal="true" aria-label="Funkcióleírások">
    ...
    <div class="modal-body">
      ...
      <div class="msec">
        ...
        <p>Ha a küszöb (pl. 3200 RPM) fölül hirtelen elveszed a gázt, motorfék közben rövid szikraelvételek jönnek, és a keverék a forró kipufogóban durran el. Gyorsításkor nem kapcsol be.</p>
      </div>
      <div class="msec">
        ...
        <p>• <b>Ágyúlövés:</b> kb. 1,6 s teljes tiltás (a kipufogó megtelik keverékkel), majd visszajön a szikra: egy nagy dörrenés és tűzgolyó.</p>
      </div>
      <div class="msec">
      ...
      <div class="msec">
        <h3>💾 Mentés</h3>
        <p>A változtatások azonnal, de ideiglenesen élnek. A kiemelt mentés gomb jelzi a nem mentett változást; a „mentve” üzenet a vezérlő visszaigazolása után jön.</p>
      </div>
      <div class="msec">
...
// Warning and red zones and the readout colors follow the configured redline
...
function renderCut() {
  ...
  else if (key === '' && now < cutHoldUntil) return;   // on a limiter the cut toggles fast: keep it readable
...
// The socket is closed during a firmware upload (frees the ESP) and resumed afterwards
...
function setConn(state) {
  ...
  txt('statusText', wsSuspended ? 'FRISSÍTÉS…' : { connecting: 'KERESÉS…', up: DEMO ? 'DEMÓ' : 'KAPCSOLÓDVA', down: 'KERESÉS…' }[state]);
}
function linkUp() {
  S.connected = true;
...
function onLaunchTap() {
  const now = Date.now();
  if (now - L.lastTapAt < 400) return;   // second tap of a double tap: ignore (no accidental disarm)
  L.lastTapAt = now;
...
function renderLaunch() {
  ...
  if (!S.connected) {
    ...
    if (L.showHeld) {
      ...
      sub = S.reason === 7 ? 'PADTESZT • ÁLLÓ MOTOR, FOLYAMATOS TILTÁS' : S.reason === 8 ? 'ANTI-FLOOD ZÁR • ENGEDD EL' : 'ENGEDD EL A LEÁLLÍTÁSHOZ';
...
// ---- Settings: SET_CFG (throttled, trailing send), SAVE_FLASH -> ACK:SAVED
...
document.querySelectorAll('[data-sl]').forEach(d => {
  ...
  const stepBtn = (sign, label) => `<button class="step" data-t="${id}" data-d="${sign}${step}" aria-label="${title}: ${label}">${sign === '-' ? '−' : '+'}</button>`;
...
// dirty = changed by the user since the last confirmed save; pend = changed but not sent yet
...
// Only changed keys are sent, at most 10 SET_CFG/s (the trailing send has the latest values),
// and never before this connection's CFG arrived, so page defaults can't overwrite the device.
function queueSend() {
...
function onConfig(json) {
  ...
  // Device values win, except keys the user changed and has not saved: those are sent again
  // (the ESP may have rebooted and lost them)
...
function saveToFlash() {
  ...
  renderSave();
  // small gap after the last SET_CFG so the two frames reach the firmware separately;
  // the ESP may answer up to ~1 s later (it waits for a moment without spark cut)
  setTimeout(() => {
    if (!saving) return;
...
function applyInfo(i) {
  ...
  // prefill the ESP Wi-Fi form, but never overwrite what the user is editing
...
// Status line + progress bar used by all three update paths
function otaUi(id) {
...
// Browser upload to POST /update (multipart field "update"); ?md5= lets the ESP verify the image
function flashBlob(blob, md5, ui) {
  ...
  return new Promise((resolve, reject) => {
    ...
    x.onerror = x.ontimeout = () => {
      if (sentAll) { resolve(); return; }   // the reboot can cut the reply short: the version check decides
      suspendWS(false);
...
// After flashing: wait for the reboot, then confirm the running version via /api/info
async function waitForVersion(expect, ui) {
  ...
  while (Date.now() - t0 < 90000) {
    ...
    if (i) {
      if (!expect || (+i.code || verCode(i.fw)) >= expect) {
      ...
      if (Date.now() - t0 > 12000) {
        suspendWS(false);
        throw new Error('A vezérlő újraindult, de még a v' + i.fw + ' fut – az új firmware nem aktiválódott. Próbáld újra.');
      }
    }
    ui.set('Újraindulás – várakozás a vezérlőre… (' + Math.round((Date.now() - t0) / 1000) + ' s)', 'info');
    await sleep(1500);
  }
  suspendWS(false);
  throw new Error('A vezérlő 90 mp alatt nem jelentkezett. Csatlakozz újra a Swift-PopsAndBangs Wi-Fi-hez, és töltsd újra az oldalt.');
}

// Path 1: the phone downloads from GitHub (mobile data), then uploads to the ESP.
// Public repo, anonymous: raw.githubusercontent.com sends Access-Control-Allow-Origin: *
...
async function ghFetch(path, timeout) {
  ...
  try { r = await fetch(url, { cache: 'no-store', signal: ctl.signal }); }
    ...
    throw new Error((ctl.signal.aborted
      ...
      : 'Nem érem el a GitHubot: a telefonon nincs mobilnet, vagy az autó Wi-Fi-je mellett nem használja. Androidon kapcsold be a mobilnetet, és ha a telefon rákérdez, maradj a Wi-Fi-n internet nélkül; iPhone-on ez többnyire nem megy.') + OTHER_WAYS);
  } finally { clearTimeout(tm); }
  if (r.ok) return r;
...
async function phoneInstall() {
  const m = FW.manifest;
  if (!m || FW.busy) return;
  if (!confirm('Telepíted a v' + m.version + ' firmware-t?\nA telefon letölti (' + kb(m.size) + '), és feltölti a vezérlőre, ami utána újraindul.\n\nÁlló motornál frissíts, és ne zárd be az oldalt.')) return;
  setBusy(true);
  try {
...
// Path 2: the ESP joins an internet Wi-Fi and downloads by itself; the page polls the status
...
async function espSaveCfg() {
  ...
  const ssid = $('staSsid').value.trim(), pass = $('staPass').value;
  const repo = $('espRepo').value.trim(), branch = $('espBranch').value.trim(), auto = $('espAuto').checked;
  if (ssid !== (i.staSsid || '') || pass) p.set('ssid', ssid);
...
async function espAction(kind) {
  if (kind === 'install' && !confirm('Az ESP letölti és telepíti az új firmware-t, majd újraindul (1–2 perc).\n\nKözben a telefon lecsatlakozhat az autó Wi-Fi-jéről – ez normális; ha nem jön vissza magától, csatlakozz újra.\n\nÁlló motornál frissíts. Indulhat?')) return;
  try {
    ...
    // the ESP may switch Wi-Fi channel right away and the reply gets lost: keep watching the status
...
async function pollTick() {
  ...
  if (s) renderOta(s);
  else if (poll.fails > 1) p2.set('⚠ Nincs kapcsolat a vezérlővel. Ha az ESP épp a másik Wi-Fi-hez csatlakozik, a telefon lecsatlakozhatott – várok és újrapróbálom… (' + poll.fails + ')', 'warn');
...
function renderOta(s) {
  ...
  // done, or the ESP rebooted into the new version while the phone was away
...
async function fileInstall() {
  ...
  if (!confirm('Feltöltöd és telepíted: ' + pickedFile.name + '?\nA vezérlő utána újraindul. Álló motornál frissíts, és ne zárd be az oldalt.')) return;
  setBusy(true);
...
// ---- Demo mode (file:// or ?demo=1): simulated engine, firmware and HTTP API; no network
const sim = DEMO ? makeSim() : null;
function makeSim() {
  const s = {
    online: true, cfg: Object.assign({}, CFG_DEF), rpm: 850, gas: false, btn: false, btnAt: 0, thr: false, liftAt: 0,
  ...
  // ~30 Hz engine model: launch states, cut priority and anti-flood like the firmware
  s.tick = () => {
    if (!s.online) return;
    const now = Date.now(), c = s.cfg;
    if (s.btn && now - s.btnAt > 600) s.btn = false;   // dead-man, like the firmware
    // the demo driver floors it when armed, drops the clutch after ~2.5 s, lifts after the launch
  ...
  s.reboot = t => {
    ...
    setTimeout(() => {
      ...
      s.ls = 0; s.btn = false; s.online = true;
      linkUp();
    }, 5000);
  };
  ...
  s.api = async (path, o) => {
    ...
    switch (path) {
      case '/api/info': return Object.assign({}, s.info, { rpm: Math.round(s.rpm), heap: 176000 + (Math.random() * 6000 | 0) });
  ...
  // GitHub mock: manifest, then a streamed fake image that starts with 0xE9
  s.gh = async path => {
    await sleep(500);
...
document.querySelectorAll('details.card').forEach(d => {
  ...
  d.addEventListener('toggle', () => { store.set('open.' + d.id, d.open ? '1' : '0'); if (d.open && (d.id === 'grpFw' || d.id === 'grpSys')) refreshInfo(); });
});
...
// Web fonts only after load and never blocking: the car's Wi-Fi has no internet
