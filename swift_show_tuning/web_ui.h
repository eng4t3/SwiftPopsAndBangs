...
async function logRefresh(force) {
  const st = await api('/api/log/status', { timeout: 3000 }).catch(e => e.message), on = st.enabled !== false;
  $('logEn').disabled = typeof st !== 'object';
  if (typeof st !== 'object') return logUi.set(st || 'Ezen a firmware-en nincs adatnapló (v2.1 kell).', 'warn');
  if (!logBusy) logEnUi(on);
  const pf = st.poolFree || {}, low = pf.cap === 0 || pf.drive === 0, n = st.captures + ' esemény, ' + st.drives + ' út';
  if (!on) logUi.set('⏸ KIKAPCSOLVA – nem ír a flash-be • ' + n + ' megmaradt', 'warn');
  else if (!st.ok) logUi.set('⚠ A naplózó nem fut (flash-hiba: ' + (st.flashErrors | 0) + ')', 'err');
  else logUi.set([st.flash ? '✔ Flash rendben' : '⚠ Flash hiba, csak RAM-puffer', n, st.writing ? '● felvétel fut' : '⏸ írás szünetel',
    st.pending && st.pending + ' mentésre vár', st.flashErrors && st.flashErrors + ' flash-hiba',
  ...
  txt('sumLog', !on ? 'KI' : (st.writing ? '● ' : '') + st.captures + ' esemény');
  $('sumLog').className = 'sv' + (on ? '' : ' off');
...
async function logToggle(on) {
  logEnUi(on);
  logBusy = true;
  const r = await api('/api/log/config', { method: 'POST', body: 'enabled=' + (on ? 1 : 0), timeout: 5000 }).catch(e => ({ ok: false, msg: e.message }));
  logBusy = false;
  if (r.ok === false) { logEnUi(!on); toast('Nem sikerült: ' + (r.msg || 'hiba'), 'err'); }
  else toast(on ? '● Naplózás bekapcsolva' : '⏸ Naplózás kikapcsolva');
  logRefresh();
}
...
function makeSim() {
  ...
  s.api = async (path, o) => {
    ...
    switch (path) {
      ...
      case '/api/log/status': return { ok: true, enabled: s.logOn, flash: true, captures: s.caps.length, drives: s.drives.length, poolFree: { cap: 20, drive: 32 }, writing: s.logOn && !s.cutStart && !s.ls, boot: 12 };
      case '/api/log/config': s.logOn = p.get('enabled') === '1'; return { ok: true, enabled: s.logOn };
      case '/api/log/list': return { captures: s.caps, drives: s.drives };
      case '/api/log/snap': if (!s.logOn) return bad('A naplózás ki van kapcsolva');
        s.caps.unshift({ id: s.nextId, boot: 12, type: 'manual', t0: Date.now() - s.bootAt, dur: 60000, sum: 'Kézi mentés' }); return { ok: true, id: s.nextId++ };
