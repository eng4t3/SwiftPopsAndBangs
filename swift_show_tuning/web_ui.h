  ...
  <details class="card" id="grpFw">
    ...
    <div class="cb">
      ...
      <div class="box">
        ...
        <p class="hint">A telefon mobilneten letölti a GitHubról a legújabb firmware-t, és feltölti a vezérlőre. Androidon az autó Wi-Fi-je mellett is megy, ha be van kapcsolva a mobilnet; iPhone-on gyakran nem – ott használd a Wi-Fi-s módot vagy a kézi feltöltést.</p>
        <div class="btn-row">
      ...
      <details class="box" id="p2Box">
        ...
        <div class="sub-b">
          <p class="hint">Az ESP egy internetes Wi-Fi-re (router vagy telefonos hotspot) csatlakozik, és maga tölti le a frissítést. Az autó Wi-Fi-je közben egy pillanatra megszakadhat (csatornaváltás) – az oldal újracsatlakozik. Hotspotnál a telefon általában nem maradhat az autó Wi-Fi-jén: indítsd el a telepítést, az ESP egyedül befejezi, utána csatlakozz vissza a Swift-PopsAndBangs hálózatra.</p>
...
<div id="infoModal" class="modal" aria-hidden="true">
  <div class="modal-card" role="dialog" aria-modal="true" aria-label="Funkcióleírások">
    ...
    <div class="modal-body">
      ...
      <div class="msec">
        <h3>💾 Mentés</h3>
        <p>A változtatások azonnal, de ideiglenesen élnek; a kiemelt gombbal mentsd őket. A „mentve” jelzés a vezérlő visszaigazolása után jön.</p>
      </div>
      <div class="msec">
...
// Only changed keys, <= 10 SET_CFG/s (trailing send = latest values), and never before this
// connection's CFG arrived, so page defaults can't overwrite the device.
function queueSend() {
...
function onConfig(json) {
  ...
  // device values win, except unsaved user changes: those are re-sent (the ESP may have rebooted)
...
function saveToFlash() {
  ...
  renderSave();
  // gap after the last SET_CFG so the frames arrive separately; the ACK may take ~1 s
  setTimeout(() => {
    if (!saving) return;
...
// `uptime` (ms since boot) < time since the upload; no `uptime` (old firmware): soft message.
...
// Path 1: the phone downloads from GitHub (raw.githubusercontent.com allows CORS), then uploads
...
async function espSaveCfg() {
  ...
  if (pass || open) p.set('pass', pass);   // empty field without "open" = keep the saved password
  if (repo && repo !== i.repo) {
...
function makeSim() {
  ...
  s.tick = () => {
    ...
    // demo driver: full throttle when armed, clutch drop after ~2.5 s, lift after the launch
