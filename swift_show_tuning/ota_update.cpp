...
//        "autoCheck":false,"rpm":0}                 (the Wi-Fi password is never returned)
//  GET  /api/ota/status
...
//                         corrupt image / no file; 409 engine above OTA_MAX_INSTALL_RPM or an
//                         online update running; 500 flash error. Plain text, Hungarian.
//
// Cross-site protection: every state-changing route (POST /update, POST /api/ota/*) answers
// 403 when an Origin header is present and its host differs from the Host header (a form on
// some other web page open on a phone joined to the car Wi-Fi). No Origin (curl, laptop
// upload) is allowed. DEPENDENCY: WebServer only keeps headers named in collectHeaders();
// the glue (.ino) must include "Origin" in its server.collectHeaders() list, otherwise
// header("Origin") is always "" and this check cannot trigger.
//
...
// markAppValid() does it once the image has proven itself.
extern "C" bool verifyRollbackLater() { return true; }
#endif

static bool appValidDone = false;   // loop task only

static void markAppValid(const char* why) {
...
// Same-origin or no Origin header (curl / laptop upload) -> allowed. Needs "Origin" in the
// glue's server.collectHeaders() list (see the header comment).
static bool originAllowed() {
  String origin = srv->header("Origin");
  if (origin.length() == 0) return true;
  String host = srv->hostHeader();
  int p = origin.indexOf("://");
  if (p <= 0 || host.length() == 0) return false;   // includes "Origin: null"
  String scheme = origin.substring(0, p);
  String originHost = origin.substring(p + 3);
  scheme.toLowerCase();
  originHost.toLowerCase();
  host.toLowerCase();
  if (scheme == "http" && originHost.endsWith(":80")) originHost.remove(originHost.length() - 3);
  if (host.endsWith(":80")) host.remove(host.length() - 3);
  if (originHost == host) return true;
  Serial.printf("[OTA] Cross-site request refused: Origin %s, Host %s\n", origin.c_str(),
                srv->hostHeader().c_str());
  return false;
}
...
static void handleInfo() {
  markAppValid("/api/info served");   // the dashboard reached us: the new image works
  OtaConfig c = copyConfig();
  ...
  snprintf(body, sizeof(body),
           ...
           "\"branch\":\"%s\",\"staSsid\":\"%s\",\"autoCheck\":%s,\"rpm\":%d}",
...
static void otaTask(void*) {
  ...
  do {
    ...
    if (rpm > OTA_MAX_INSTALL_RPM) {
      snprintf(err, sizeof(err), "A motor jár (%d RPM). Frissíteni csak alapjáraton vagy álló motorral lehet (max. %d RPM).",
               rpm, OTA_MAX_INSTALL_RPM);
      break;
    }
...
static bool startJob(bool install, bool automatic, char* why, size_t whyLen) {
  ...
  if (install) {
    int rpm = engineGetRpm();
    if (rpm > OTA_MAX_INSTALL_RPM) {
      snprintf(why, whyLen, "A motor jár (%d RPM). Frissíteni csak alapjáraton vagy álló motorral lehet (max. %d RPM).",
               rpm, OTA_MAX_INSTALL_RPM);
      return false;
    }
  }
...
static void handleInstall() {
  if (rejectCrossSite()) return;
  markAppValid("install requested");   // never replace an unconfirmed image
  char why[200];
  if (startJob(true, false, why, sizeof(why))) sendOk(); else sendFail(why);
}
...
// WebServer handles the whole multipart body inside one handleClient() call, so loop() (and
// engineUpdate()) is blocked meanwhile. The engine is therefore inhibited for the whole
// request, including a rejected one whose body is still being drained.
...
static void upFail(int http, const char* msg) {
  upReject(http, msg);
  if (uploadActive) {
    if (Update.isRunning()) Update.abort();
    setStatus(ST_ERROR, 0, "%s", upMsg);
  }
}
...
static void handleUploadChunk() {
  ...
  switch (up.status) {
    case UPLOAD_FILE_START: {
      ...
      upHttp = 0;
      upMsg[0] = 0;
      holdUpload = true;          // loop() is blocked from now on: keep the ignition stock
      applyInhibit();

      if (!originAllowed()) { upReject(403, CROSS_SITE_MSG); return; }
      int rpm = engineGetRpm();
      if (rpm > OTA_MAX_INSTALL_RPM) {
        char m[200];
        snprintf(m, sizeof(m), "A motor jár (%d RPM). Frissíteni csak alapjáraton vagy álló motorral lehet (max. %d RPM).",
                 rpm, OTA_MAX_INSTALL_RPM);
        upReject(409, m);
        return;
      }
      markAppValid("upload requested");   // never replace an unconfirmed image
      char md5[40] = "";
...
static void handleUploadDone() {
  if (!upSeen) {
    holdUpload = false;
    applyInhibit();
    if (!originAllowed()) return sendText(403, CROSS_SITE_MSG);
...
void otaLoop() {
  ...
  if (!appValidDone && millis() > OTA_MARK_VALID_MS && (WiFi.getMode() & WIFI_MODE_AP)) {
    markAppValid("uptime with AP up");
  }
